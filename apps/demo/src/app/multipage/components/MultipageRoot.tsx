...
export function usePreventPanOnDrag() {
  const isDragging = useCustomPuck((s) => s.appState.ui.isDragging);
  return !isDragging;
}
...
export const MultipageRoot: PuckComponent<MultipageRootProps> = ({
  ...
  const [nodes, setNodes, onNodesChange] = useNodesState<
    Node<{ page: PageItem; index: number; slot: SlotComponent }>
  >([]);
  const [, , onEdgesChange] = useEdgesState<Edge>(initialEdges);
    ...
    setNodes((prevNodes) =>
      validPages.map((page, i) => {
        ...
        const initialPosition = {
          x: (i % cols) * 900,
          y: Math.floor(i / cols) * 1300,
        };

        return {
          id: page.id,
          position: existingNode ? existingNode.position : initialPosition,
          ...
          selectable: false,
          draggable: false,
          ...(existingNode && existingNode.measured
  ...
  // Dynamic Layout Effect based on measured node heights
  React.useEffect(() => {
    ...
    const cols = 3;
    const gapX = 900;
    const gapY = 150; // Spacing between rows

    const rowHeights: number[] = [];

    // First pass: Calculate maximum height for each row
    nodes.forEach((node) => {
      const i = node.data.index as number;
      const row = Math.floor(i / cols);
      const height = node.measured?.height ?? 1200; // fallback height
      rowHeights[row] = Math.max(rowHeights[row] || 1200, height);
    });

    // Second pass: Update positions if they deviate
